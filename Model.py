...
#%%-----------------------------Modèle stationnaire homogène-----------------
N = 10
T_statH = np.zeros([11])
TN = 220
Tab =[0]*11
for n in range(0,N+1):
    T_statH[n] = (N+1-n)**(1/4)*TN
    Tab[n]=n
plt.title("Température de chaque couche n",fontsize=30)
plt.tick_params(axis='x', labelsize=20)
plt.tick_params(axis='y', labelsize=20)
plt.xlabel('Temperature T(K)',fontsize = 22)
plt.ylabel('Couche n',fontsize = 22)
plt.plot(T_statH,Tab)
plt.figure()
...
Ed = np.zeros([11]) # Radiation descendant (down)
Eu = np.zeros([11]) # Radiation ascendant (up)
eps = np.zeros([11])
T=np.zeros([100000,11])
dT= np.zeros([11])
...
plt.plot(temps,T[:,0],label='n=0')
plt.plot(temps,T[:,1],label='n=1')
plt.plot(temps,T[:,2],label='n=2')
plt.plot(temps,T[:,3],label='n=3')
plt.plot(temps,T[:,4],label='n=4')
plt.plot(temps,T[:,5],label='n=5')
plt.plot(temps,T[:,6],label='n=6')
plt.plot(temps,T[:,7],label='n=7')
plt.plot(temps,T[:,8],label='n=8')
plt.plot(temps,T[:,9],label='n=9')
plt.plot(temps,T[:,10],label='n=10')
plt.legend(fontsize = 19)
